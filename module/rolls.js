  ...
  static async rollMove(options = {}) {
    ...
    if (item) {
      ...
      if (item.type == 'move') {
        ...
        // If this is an ASK roll, ask user to pick a stat.
        if (data.roll == 'ask') {
        ...
        // If this is a BOND roll, let the user enter the bond value.
        ...
        // If this is an ASKMOD roll,let the user enter their value.
	// (this is the same as a bond, but different semantics.)
            ...
            new Dialog({
              ...
              buttons: {
                submit: {
                  label: 'Roll',
                  callback: html => {
                    this.rollMoveExecute('askmod', data, templateData, html[0].querySelector("form"))
                  }
                }
              }
            }, dlgOptions).render(true);
  ...
  static async rollMoveExecute(roll, dataset, templateData, form = null) {
    ...
    if (!DwUtility.isEmpty(roll)) {
      ...
      if (!validRoll || dataset?.rollType == 'formula') {
        ...
        else if (roll.toLowerCase() == 'askmod') {
          formula = form.bond?.value ? `${dice}+${form.bond.value}` : dice;
          if (dataset.value && dataset.value != 0) {
            formula += `+${dataset.value}`;
          }
        ...
        // Handle adv/dis.
        let rollMode = this.actor.data.flags?.paranoidworld?.rollMode ?? 'def';
        switch (rollMode) {
          case 'adv':
            rollModeUsed = true;
    ...
    if (game.combat && game.combat.combatants) {
      ...
      if (combatant) {
        let moveCount = combatant.data.flags.paranoidworld ? combatant.data.flags.paranoidworld.moveCount : 0;
        moveCount = moveCount ? Number(moveCount) + 1 : 1;
        // Emit a socket for the GM client.
        if (!game.user.isGM) {
          game.socket.emit('system.paranoidworld', {
            combatantUpdate: { _id: combatant.id, 'flags.paranoidworld.moveCount': moveCount }
          });
        }
        else {
          await game.combat.updateEmbeddedDocuments('Combatant', [{ _id: combatant.id, 'flags.paranoidworld.moveCount': moveCount }]);
          ui.combat.render();
        }
      }
    }
    ...
    if (forwardUsed || rollModeUsed) {
      ...
      if (rollModeUsed && game.settings.get('paranoidworld', 'advForward')) {
        updates['flags.paranoidworld.rollMode'] = 'def';
      }
