...
export class PwItemSheet extends ItemSheet {
  ...
  static get defaultOptions() {
    let options = mergeObject(super.defaultOptions, {
      classes: ["paranoidworld", "sheet", "item"],
      width: 520,
      height: 480,
  ...
  /** @override */
  get template() {
    const path = "systems/paranoidworld/templates/items";
  ...
  async _onClickClassControl(event) {
    ...
    <textarea class="${field_types[field_type]}" name="data.${field_type}.${nk}.description" rows="5" title="What's your ${field_types[field_type]}?" data-dtype="String"></textarea>
        ...
        const nk = Object.keys(field_values).length + 1;
        let template = '/systems/paranoidworld/templates/items/_class-sheet--equipment-group.html';
        let templateData = {
          group: nk
        };
        let newKey = document.createElement('div');
  ...
  _updateObject(event, formData) {
    ...
    if (typeof formObj.data.drives == 'object') {
      for (let [k, v] of Object.entries(formObj.data.drives)) {
        if (i != k) {
          formObj.data.drives[i] = v;
          delete formObj.data.drives[k];
          deletedKeys.push(`drives.${k}`);
        }
        i++;
      }
    }
